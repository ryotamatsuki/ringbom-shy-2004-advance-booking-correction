...
- Run 3, commit `9afe19584b974c9f46d1a88d7c286a3ded82c32b`, completed the Lean 4.34.0 / mathlib build successfully, then failed because the repository placeholder scanner also inspected `.lake/packages` and found test/example placeholders in pinned dependencies. The project theorem sources were not implicated. The scanner now excludes `.lake`, and the axiom script rejects every nonstandard axiom in the printed theorem certificates; a fourth fresh run is required.
