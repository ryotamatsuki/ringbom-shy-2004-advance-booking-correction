...
The omission can reverse the endpoint ranking even when both rates lie strictly inside the feasible interval. At the exact parameter point reported below, the printed condition favors $r_L$, while primitive expected profits favor $r_H$. A second issue concerns global choice. The endpoint comparison does not establish that either rate is optimal when a threshold is outside $[0,1]$ or when an available no-reservation interval yields zero while feasible endpoint profits are negative. We give a complete correspondence on the source's refund strategy set under an explicit weak-participation convention consistent with the source's continuum cutoff.

The missing-factor error is confined to the two-type comparison and does not propagate into the later uniform-type derivation. We independently rederive the uniform-type formulas and welfare results on their active domain. Those results survive, subject to their domain qualifications, but two numerical private-refund entries in Table 1 are independently incorrect. We report those entries separately so that the correction does not overstate what survives.

\section{Two-type model}

There are two consumer types, $H$ and $L$, with show probabilities $0<\sigma_L<\sigma_H<1$ and population shares $\alpha_H,\alpha_L>0$, where $\alpha_H+\alpha_L=1$. The seller chooses a refund rate $r\in[0,1]$. A reservation is paid for in advance at price $p$. If the consumer shows, she receives value $\beta$ and the seller keeps $p$; if she cancels, she receives refund $rp$ and forfeits $(1-r)p$. Consistently with the source's state-contingent profit equations, the seller's cost $c$ is charged in either state and salvage value $s$ is added after cancellation, giving seller payoffs $p-c$ on show and $(1-r)p+s-c$ on cancellation. We adopt weak participation, $U_i(r)\geq0$, which is consistent with the source's continuum cutoff; the source does not separately state a tie rule.
...
\end{equation}
When $\beta>p$, our $r_H$ and $r_L$ correspond to the source's $r_1$ and $r_2$, respectively. Thus the ordering $r_H<r_L$ requires $\beta>p$. Thresholds must still be checked against the strategy set; a raw negative threshold is not a feasible refund.

\section{Correct endpoint comparison}

Suppose $0\leq r_H<r_L\leq1$. Under weak participation, only type $H$ reserves at $r_H$, and both types reserve at $r_L$. Let $\Pi_H=n\alpha_Hg_H(r_H)$ and $\Pi_{HL}=n[\alpha_Hg_H(r_L)+\alpha_Lg_L(r_L)]$, where $n>0$ is the potential market size.
...
\begin{proof}
At $r_L$, the seller earns $q_L$ from each type-$L$ reservation by \eqref{eq:q}. Type $H$'s change in unit profit between the rates is $-(1-\sigma_H)p(r_L-r_H)$. Substituting \eqref{eq:thresholdgap} gives
\[
 (1-\sigma_H)p(r_L-r_H)
...
\begin{equation}
 ...
 \label{eq:slope}
\end{equation}
Thus profit falls strictly between threshold events. The expressions displayed in the source's equation (4) substitute threshold values; they are endpoint profits rather than the profit at every interior refund in each region. For the global characterization below, zero population weights are allowed only to cover degenerate boundaries explicitly; this relaxes the positive-share assumption used in the two-type endpoint comparison.

\begin{proposition}[Complete feasible refund correspondence]
\label{prop:global}
Let $n>0$, $\beta,p>0$, $c,s\geq0$, $0\leq\sigma_L<\sigma_H<1$, $\alpha_i\geq0$ with $\alpha_H+\alpha_L>0$, and use \eqref{eq:utility}--\eqref{eq:profit}. The weights need not be normalized; normalizing them only rescales profit. Let $J=\{i:\alpha_i>0\}$, $\rho_i=(p-\beta\sigma_i)/(p(1-\sigma_i))$, and
\[
...
\begin{proof}
Each utility is affine and strictly increasing in $r$, so a type joins at its raw threshold if that threshold lies in $[0,1]$, is already present at $r=0$ if its threshold is negative, and never joins if its threshold exceeds one. The candidate set includes zero and every in-domain participation threshold of a positive-mass type. On every interval to the right of a candidate on which the participating set is fixed and nonempty, \eqref{eq:slope} makes profit strictly decreasing, so every noncandidate point in that interval is dominated by its left endpoint candidate, irrespective of whether profits are positive, zero, or negative. The same argument applies after the last threshold event. Therefore the only possible noncandidate maximizers lie on an initial interval with no participants. If every positive-mass threshold is positive, that no-reservation set $\{r\in[0,1]:r<\rho_0\}$ has profit zero and belongs to the argmax exactly when $M=0$. Weak acceptance evaluates threshold equalities at the threshold itself, including $r_i=0$ and $r_i=1$. In particular, at $\beta=p$ both thresholds equal one; for $\beta<p$, positive-show-probability types have thresholds above one. The argument uses primitive utility and does not divide by $\beta-rp$, so it also covers the boundary $\beta-rp=0$.
\end{proof}

When $0<r_H<r_L<1$ and both masses are positive, the candidate set is $\{0,r_H,r_L\}$. The following profitability condition gives the global qualification needed when reading the source's Corollary 1; it does not restate all four comparative-static directions as new theorems.

\begin{corollary}[Both types served]
On $0<r_H<r_L<1$, $r_L$ belongs to the seller's optimal-refund correspondence if and only if
\begin{equation}
...
The source's four comparative-static directions---higher $p$, lower $\alpha_H$, lower $\sigma_H$, or higher $\sigma_L$---therefore require the ordered-threshold domain and the profitability condition in \eqref{eq:correctioncor}; an improving endpoint comparison does not by itself make either endpoint beat an available zero-profit no-reservation plateau. The high-price claim fails without this qualification even arbitrarily close to $\beta$. Fix $\beta=1$, $c=9/10$, $s=0$, equal masses, and $(\sigma_H,\sigma_L)=(1/2,1/10)$. For every $p\in(1/2,1)$, $0<r_H<r_L<1$. At $r_H$, $q_H=-2/5$, so $\Pi_H/n=-1/5$. At $r_L$, the seller's high-type unit profit is below $q_H$ and $q_L=-4/5$, hence $\Pi_{HL}/n<-1/5$, while every $r<r_H$ yields no reservations and zero profit. Thus there is no threshold below $\beta$ at which raising $p$ sufficiently close to $\beta$ alone guarantees that both types are served.
...
\]
Let $B(r)$ denote the bracketed term. On the stated strict domain, $\beta-rp>0$ on $[0,1]$, $B(r)$ is strictly decreasing, and $B(1)=s-c-(\beta-p)/2<0$. Hence the derivative changes sign at most once and cannot remain positive at the upper boundary: if $B(0)\leq0$ the constrained optimum is $0$, while otherwise the unique root lies in $(0,1)$. Solving for that root gives
\[
 r_{\mathrm{int}}=\frac{\beta[3p-2(c-s)]-2ps-\beta^2}{p(\beta-2c+p)},\qquad
 \bar r=\max\{0,r_{\mathrm{int}}\}.
\]
Recomputing this formula and primitive profits gives $\bar r=8/13$ rather than the printed $0.250$ at $(\beta,p,c,s)=(1,1/2,1/10,1/10)$, and $\bar r=2/3$ rather than $0.357$ at $(1,1/2,0,0)$.

The planner's objective is
\[
 \frac{W(r)}n=\int_{\widehat\sigma(r)}^1[(\beta-s)\sigma-(c-s)]\,d\sigma,
\]
because refund transfers cancel between consumers and the seller. The welfare integrand changes sign once, at $\widehat\sigma^*=(c-s)/(\beta-s)$. This cutoff is feasible on the stated domain: $0\leq\widehat\sigma^*<p/\beta$ because $p(\beta-s)-\beta(c-s)=\beta(p-c)+s(\beta-p)>0$. It is therefore the global welfare-maximizing cutoff, giving
\[
...
\[
 ...
 \end{cases}
\]
In particular, $r^*>\bar r$ on the stated strict domain. If the assumption $p>c$ is relaxed, strictness is no longer uniform: at $p=c,s=0$ both rates can equal zero, while other boundary points can remain strict. All remaining Table 1 values and all Table 2 welfare-loss entries reproduce at the published precision when evaluated at the primitive optimum. The uniform cutoff expression is not used at $\beta-rp=0$; boundary cases require direct utility rather than division by a zero denominator.

\section{Conclusion}

The correct two-type endpoint comparison adds the factor $1/(1-\sigma_L)$ to the high-type loss term in equation (6). Its omission can reverse the ranking of feasible refund endpoints. A complete global choice also evaluates threshold feasibility, the explicit weak-participation rule, and any available no-reservation plateau. These corrections qualify Proposition 1 and Corollary 1, while the later uniform welfare analysis remains valid on its active domain. Two numerical private-refund entries in Table 1 require correction.
